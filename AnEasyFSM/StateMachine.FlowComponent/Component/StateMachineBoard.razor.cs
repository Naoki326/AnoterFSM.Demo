...
namespace StateMachine
{
    ...
    public partial class StateMachineBoard
    {
        ...
        public async Task ChangeNodeNameAsync(string oldName, string newName)
        {
            ...
            if (await _drawflow.GetNodeFromIdAsync<NodeData>(GetFlowIndexStr(engine[oldName].FlowID)) is StateMachineFlowNode<NodeData> nodeInput
        ...
        public async Task RemoveNodeAsync(string nodeName)
        {
            ...
            var nodeId = GetFlowIndexStr(engine[nodeName].FlowID);
            await _drawflow.RemoveNodeAsync(nodeId);
        }

        private void NodeRemoved(string nodeId)
        {
            if (engine.FirstOrDefault(p => GetFlowIndexStr(engine[p.Name].FlowID) == nodeId) is IFSMNode nd)
            {
        ...
        public void OpenChangeNodeNameDialog(string nodeName)
        {
            if (!engine.ContainsNode(nodeName))
                return;
            tempNodeId = GetFlowIndexStr(engine[nodeName].FlowID);
            inputName = nodeName;
            changeNodeNameDialog = true;
        }
        ...
        public void OpenChangeConnectionNameDialog(string outputNodeName, string inputNodeName)
        {
            tempConnectArgs = new FlowConnectionArgs()
            {
                ...
                InputId = GetFlowIndexStr(engine[inputNodeName].FlowID),
                OutputId = GetFlowIndexStr(engine[outputNodeName].FlowID),
            };
        ...
        public async Task ChangeConnectionNameAsync(string fromNodeName, string toNodeName, string newName)
        {
            ...
            await _drawflow.SetConnectionNameAsync(GetFlowIndexStr(engine[fromNodeName].FlowID), GetFlowIndexStr(engine[toNodeName].FlowID), "output_1", "input_1", newName);
        ...
        public async Task RemoveConnectionAsync(string fromNodeName, string toNodeName)
        {
            ...
            await _drawflow.RemoveSingleConnectionAsync(GetFlowIndexStr(engine[fromNodeName].FlowID), engine[toNodeName].FlowID, "output_1", "input_1");
        }
